...
export function useHostCalendar() {
    ...
    async function load() {
      try {
        ...
        if (res.ok) {
          ...
          const data = Array.isArray(json.data?.properties) ? json.data.properties : [];
          if (!cancelled) {
            setListings(data);
  ...
  const fetchCalendar = useCallback(async (propertyId: string, from?: string, to?: string) => {
  ...
  const blockDates = useCallback(
    async (propertyId: string, startDate: string, endDate: string, reason?: string) => {
      await fetch(apiUrl(API_CONSTANTS.ENDPOINTS.PROPERTY_CALENDAR_BLOCK(propertyId)), {
        method: "POST",
        headers: getJsonHeaders(),
  ...
  const unblockDate = useCallback(async (propertyId: string, blockId: string) => {
    await fetch(apiUrl(API_CONSTANTS.ENDPOINTS.PROPERTY_CALENDAR_UNBLOCK(propertyId, blockId)), {
      method: "DELETE",
      headers: getJsonHeaders(),
    });
  }, []);
