        ...
        <Card className="rounded-2xl border-amber-200 bg-amber-50 shadow-none">
          <CardContent className="pt-5">
            ...
            <p className="text-sm text-amber-700 mb-3">
              You have {draftListings.length} unfinished propert{draftListings.length === 1 ? "y" : "ies"}. Continue to publish.
            </p>
            <ul className="space-y-2">
              {draftListings.map((l) => (
                <li key={l.id} className="flex items-center justify-between gap-2 border-b border-amber-200 pb-2 last:border-0">
                  <span className="font-medium text-sm truncate text-amber-900">{l.title}</span>
                  <Link
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          ...
          {recentBookings.length > 0 && (
            <Button variant="ghost" size="sm" className="text-xs text-slate-500" onClick={onViewBookings}>
              View all
            </Button>
          )}
        </div>
