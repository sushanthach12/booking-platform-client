...
export function BookingFailedScreen({ message, retryError, retrying, onRetry, onHome }: BookingFailedScreenProps) {
  return (
      ...
      <div className="w-full max-w-md">
        ...
        <div className="anim-fade-up-2 rounded-2xl border border-border bg-card shadow-sm overflow-hidden mb-4">
          <div className="bg-linear-to-r from-destructive/80 to-rose-500/80 px-5 py-3 flex items-center gap-2">
            ...
            <span className="text-white text-sm font-semibold">Payment Unsuccessful</span>
          </div>
          <div className="px-5 py-5">
            <p className="text-sm text-muted-foreground">
              Your payment was not processed. Your reservation has not been confirmed.
              You can retry with the same dates or go back to home.
            </p>
          </div>
        </div>
        ...
        {/* Retry error */}
        {retryError && (
          <p className="anim-fade-up-3 text-sm text-destructive text-center mb-4">{retryError}</p>
        )}
        ...
        <div className="anim-fade-up-3 flex flex-col gap-3">
          <Button onClick={onRetry} disabled={retrying}>
            {retrying ? (
              <><Loader2 className="size-4 mr-2 animate-spin" />Preparing payment…</>
            ) : (
              "Try again"
            )}
          </Button>
