...
import { GuestBooking, GuestBookingsSummary, GuestProfile } from "@/domain/entities";
...
function SettingsRow({
  ...
  return (
    <div className="flex items-center gap-4 py-4">
      <div className={cn("size-9 rounded-xl flex items-center justify-center shrink-0", destructive ? "bg-red-50" : "bg-slate-50")}>
        <Icon className={cn("size-4", destructive ? "text-red-500" : "text-slate-500")} />
      </div>
      <div className="flex-1 min-w-0">
        <p className={cn("text-sm font-semibold", destructive ? "text-red-600" : "text-slate-800")}>{label}</p>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
            <div className="relative">
              <Avatar className="size-20 ring-4 ring-white shadow-md">
                <AvatarImage src={localProfile.avatarUrl ?? undefined} alt={fullName} />
            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                ...
                {localProfile.isVerified && (
                  <Badge variant="outline" className="gap-1 text-emerald-700 border-emerald-200 bg-emerald-50 text-xs font-semibold">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-slate-500 mb-3">
                ...
                <span className="flex items-center gap-1">
                  ...
                  Member since {format(parseISO(localProfile.memberSince), "MMMM yyyy")}
                </span>
              </div>
              {localProfile.bio && (
                <p className="text-sm text-slate-600 max-w-lg">{localProfile.bio}</p>
              )}
            </div>
              ...
              <div className="space-y-4">
                <Card className="rounded-2xl border-slate-100 shadow-none">
                  ...
                  <CardContent className="divide-y divide-slate-50 px-6">
                    <SettingsRow icon={Mail} label="Email address" description={localProfile.email} />
                    <SettingsRow icon={Lock} label="Password" description="Last changed 3 months ago" />
                    <SettingsRow
                      icon={ShieldCheck}
